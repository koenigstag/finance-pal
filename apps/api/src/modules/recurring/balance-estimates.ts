...
// moves with the sent one (the right-hand side reads the row as it was). Not to or from nothing,
// which carries no rate: the received amount then stays as it was.
const SET_AMOUNT_SQL = `
  UPDATE transactions
  SET amount = $2,
      dest_amount = CASE
        WHEN dest_amount IS NULL THEN NULL
        WHEN amount = 0 OR $2::numeric = 0 THEN dest_amount
        ELSE GREATEST(round(dest_amount * $2::numeric / amount, 2), 0.01)
      END,
      percentage_as_of = $3
  WHERE id = $1
`;

// Worked out on its date to nothing: no money moved, so there's no transaction to keep. Its amount
// may be nothing already, which only an estimate may be, so it stays one as it goes.
const TO_NOTHING_SQL = `UPDATE transactions SET deleted_at = now() WHERE id = $1`;
 ...
 * — which may come to nothing for now. One whose date has come is worked out a last time, from the
 * balance on that date, and stays as it came out; or goes, if that's nothing. Returns what changed.
 */
export async function reworkEstimates(manager: EntityManager, accountIds: string[] | null, now: Date): Promise<ReworkedAmount[]> {
  ...
  for (const row of rows) {
    ...
    if (landed && !isPositiveMoney(amount)) {
